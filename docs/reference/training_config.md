...
- **Consolidation timing:** consolidate after a plateau (several iterations without improvement) or when the prompt exceeds a character count you're comfortable with. Consolidation does not reset training history — the optimizer still has access to prior learnings. Pass `val_bundles` to `consolidate()` to re-score the compressed prompt immediately — consolidation is lossy and a score drop means coverage was lost; without re-scoring the new version inherits the pre-compression score.
- **Default batch strategy:** `FailurePriorityBatchStrategy` when an evaluator is active — the batch from a rejected iteration is retried (mixed with unseen examples) in the next one. Without an evaluator, `RandomBatchStrategy`. Both are seeded with `config.seed` when going through `Project.train`.
- **Resumption numbering:** a resumed run continues iteration numbering from the restored training log (`max_iterations` counts iterations of the current run), so the history the optimizer sees never contains duplicate iteration numbers.
